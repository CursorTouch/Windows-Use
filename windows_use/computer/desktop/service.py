...
from windows_use.computer.tree import Tree
from windows_use.computer.watchdog import WatchDog
from windows_use.computer.desktop.config import KEY_ALIASES
from windows_use.computer.desktop.utils import escape_text_for_sendkeys
from windows_use.computer.desktop.types import Browser, DesktopState, Size, Status, Window
from windows_use.computer.tree.types import BoundingBox, TreeElementNode, TreeState
...
class Desktop:
    ...
    def start(self):
        """Start the watchdog service for monitoring UI Automation events."""
        if self.watchdog:
            self.watchdog.start()
            logger.debug("Desktop watchdog started")

    def stop(self):
        """Stop the watchdog service."""
        if self.watchdog:
            self.watchdog.stop()
            logger.debug("Desktop watchdog stopped")
